...
$(function() {
  ...
  dApplication.socketServer = "ws://nextbillionvoices.com:8080";
  ...
  dApplication.localStrings = {
    ...
    "default": {
      ...
      title: "Mozilla: The Next Billion Voices",
      ...
      textBillion: "Billion",
      textVoices: "Voices",
      copy1: "In the coming years, a billion people will add their voices to the Web.",
      copy2: "Add yours to see how the power of mobile can connect us all.",
      ...
      infoInfoCopy: "The world's gone mobile. As a new generation of users comes online, they'll join the ongoing conversation in new ways and from new places. Touch the screen or click to add your mark and watch as others do the same around the world thanks to the common language of the Web."
    }
  ...
  // some necessary top-level event bindings, the views generally hande their own from here
  $(window).bind('orientationchange', function() {
    ...
    if ($("body").hasClass("safari") && $(window).width() < 330) {
    
      $("#moz-lockup").css({ top: $(window).height() + 65 });  
    
    }
    else {
      $("#moz-lockup").css({ top: "100%" });  
    }
  ...
  /*
  for (var i=0; i<330; i++) {
    container.append("<span class='client-dot'>");
  }
  */
  ...
  if (window.location.search.indexOf("kiosk") > 0) {
    ...
    alert("moving into kiosk mode, if this isn't what you want, remove that little '?kiosk=true' from the URL!");
  ...
  openNewSocketCx: function() {
    ...
    if (SocketTransport.socket) {
      ...
      SocketTransport.socket.addEventListener("open", function(event) {
        ...
        var msg = '{ "clientID": "' + dApplication.clientID + '","type": "hello", "status": "ok", "message": "hello", "x": 0, "y": 0 }';
  ...
  processMessage: function(inMessage) {
    ...
    // a return message structure with all application statistics in one handy package
      ...
      $(inMessage.stats).each(function(inCount) {
        ...
        $("#country-scroller").append("<div class='country-block'><div class='country-image' style='background-image: url(views/info/images/countries/" + NBVUtil.countryImageMap[this.country_code] + ");'></div><div class='country-info'>" + this.country_name + "<br>" + this.city + "</div></div>");
    ...
    // a client has come on or dropped off, let's update our count
      ...
      //console.log("%%%%% clientcount received!");
      //console.log(inMessage);
...
var NBVUtil = {
  ...
  countryCodes: {
    ...
    "TT":"Trinidad &amp; Tobago", "TV":"Tuvalu", "TW":"Taiwan, Province of China", 
  ...
  playSound: function(inSoundIndex) {
    ...
    if (AudioTransport.webAudioContext) {
      ...
      if (AudioTransport.audioFileIndex == 0) { // that is, the first play
        ...
        //var audioTag = document.getElementById(soundElementId);
        //AudioTransport.playFromAudioTag(audioTag, inSoundIndex);
